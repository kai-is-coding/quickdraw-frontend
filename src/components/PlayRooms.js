...
class PlayRooms extends React.Component {
  ...
  getSelected(roleRoomId, roomRole, roleIndex){
    ...
    const {roomId, role, index, userId} = this.state.selected;
  ...
  guesserSelect = (event, roomId, role, index, userId) => {
    ...
    axios.patch(`https://quickdraw-backend.herokuapp.com/users/${this.props.userDetails.id}`,{withCredentials: true})
